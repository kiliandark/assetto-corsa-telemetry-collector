...
![Гоночный симулятор на стенде](/img/sim.jpg)
...
![Моды Assetto Corsa](/img/2106.png)
...
## Как я подошел к сбору данных

Как упоминалось выше, **Assetto Corsa** игра не новая, однако за счет модинга вокруг нее давно выросло сообщество. Оказалось, что существует несколько способов забрать телеметрию ([ACTI](https://www.overtake.gg/downloads/acti–assetto–corsa–telemetry–interface.3948/), [Simhub](https://www.simhubdash.com/)), но мне они не подошли. `ACTI` формирует данные только после окончания сессии, использует бинарные форматы и требует ручной обработки, а `SimHub` не умеет передавать телеметрию в реальном времени для первой **Assetto Corsa** (2014) в отличие от второй части **Assetto Corsa Competizione** (2019) или других гоночных симуляторов.

В итоге я остановился на чтении данных из **Shared Memory** (*выделенная область оперативной памяти, через которую игра передает текущие показатели стороннему софту в режиме реального времени*).

Я взял [библиотеку для работы с Shared Memory](https://github.com/mdjarv/assettocorsasharedmemory), доработали её и написали свой коллектор. Его задача была c определенной периодичностью читать текущие значения и отправлять их дальше в виде `JSON`.
...
Критически важным было добиться мгновенной отправки данных. Пока пилот проезжает круг, данные уже на дашборде.
...
![Дашборд: Текущий заезд 1](/img/dash_1.PNG)
...
![Дашборд: Текущий заезд 2](/img/dash_2.png)
...
![Дашборд: Сессии и ачивки 1](/img/session_1.png)
...
На дашборде отображался лидер по каждой ачивке. Если пилот удерживал первое место до конца дня, он получал мерч и аплодисменты от зрителей. Более подробно о реализации дашбордов рассказал мой коллега в своей [статье](https://habr.com/ru/articles/973024/).

![Дашборд: Сессии и ачивки 2](/img/session_2.png)
...
## Что это даёт кроме развлечения

Такой подход показывает, как любой поток событий, даже из игр можно превратить в работающую аналитическую систему. Те же принципы работают для IoT, промышленного оборудования или мобильных приложений: сбор метрик в реальном времени, мгновенная визуализация и вовлечение пользователей.

Наш стенд был развлечением, но под капотом — полноценная production-архитектура, готовая к масштабированию.

[Читать на Хабре](https://habr.com/ru/articles/995432/)